"""
Tests for budget_parse — unit tests for each composable component + integration.
"""
...
from pdf_cell_detect import Cell, extract_geometry_from_path, build_grid, assign_words_to_cells
...
class TestParseSetsumeiLine:
    ...
    def test_text_with_number_suffix(self) -> None:
        ...
        # "1人" is not a pure number, so it's plain text
...
class TestContinuation:
    def test_continuation_detected(self) -> None:
        ...
        result = classify_row(idx, 1, frozenset())
    ...
    def test_collect_stops_at_non_continuation(self) -> None:
        idx = build_cell_index(CONTINUATION_CELLS)
        full = collect_full_name(idx, 0, (1, 2))
        # Row 2 is sub_item (has kingaku), not continuation
...
class TestGroupByMoku:
    ...
    def test_orphan_before_moku(self) -> None:
        ...
        groups = group_rows_by_moku(classified)
        assert len(groups) == 2
        # First group: orphan (no moku)
        assert groups[0][0] is None
        assert groups[0][1] == (1, 2)
        # Second group: moku at row 3
        assert groups[1][0] == 3
        assert groups[1][1] == (4,)
...
class TestGroupBySetsu:
    def test_basic(self) -> None:
        idx = build_cell_index(SAMPLE_CELLS)
        # Rows 1 (setsu: "1 報酬"), 2 (setsu: "4 共済費"), 3 (setsumei), 4 (setsumei)
        groups = group_rows_by_setsu(idx, (1, 2, 3, 4))
        assert len(groups) == 2
        assert groups[0][0] == 1  # "1 報酬"
        assert groups[1][0] == 2  # "4 共済費"
        assert 3 in groups[1][1]
        assert 4 in groups[1][1]

    def test_orphan_before_setsu(self) -> None:
        # Rows with no setsu first
        cells = (
        ...
        assert len(groups) == 2
        assert groups[0][0] is None  # orphan
        assert groups[0][1] == (0,)
        assert groups[1][0] == 1
...
class TestBuildSetsuRecord:
    def test_basic_setsu(self) -> None:
        idx = build_cell_index(SAMPLE_CELLS)
        # Row 2: "4 共済費", 127; children: rows 3, 4 (setsumei)
    ...
    def test_setsu_with_continuation(self) -> None:
        ...
        # Row 2 is sub_item after skipping continuation row 1
...
class TestBuildMokuRecord:
    def test_from_sample(self) -> None:
        idx = build_cell_index(SAMPLE_CELLS)
        # Moku at row 1, children: rows 2, 3, 4
        record = build_moku_record(idx, 1, (2, 3, 4))
        assert record.name == "11 会計管理費"
        assert record.honendo == 85912
        # Row 1 also has right-table data ("1 報酬"), so it's included
        assert len(record.setsu_list) >= 2  # "1 報酬" and "4 共済費"
...
# ---------------------------------------------------------------------------
# parse_page_budget (top-level composition)
# ---------------------------------------------------------------------------
...
class TestIntegration106:
    ...
    def test_setsu_setsumei(self) -> None:
        moku = self.budget.moku_records[0]
        houshuu = next(s for s in moku.setsu_list if s.name == "報酬")
        # Should have "001 給与費 998" in setsumei
    ...
    def test_orphan_setsu_exist(self) -> None:
        # Rows before 目 on this page (from previous page's 目)
        assert len(self.budget.orphan_setsu) >= 1

    def test_orphan_setsu_content(self) -> None:
        # Should include 需用費, 委託料, etc. from the top half
    ...
    def test_continuation_name(self) -> None:
        # "18 負担金、補助" + "及び交付金" should be merged
        all_setsu = (
    ...
    def test_sub_items(self) -> None:
        moku = self.budget.moku_records[0]
        # 旅費 should have sub-items: 費用弁償, 普通旅費, 特別旅費
    ...
    def test_row_classification_coverage(self) -> None:
        ...
        # Should have at least these kinds
