"""
Budget Table Parser — Pure functions for structuring detected cells into budget records.
...
Each function is a small, testable, referentially transparent transform.
IO is not present in this module.
"""
...
from pdf_cell_detect import Cell
...
def parse_setsumei_line(text: str) -> SetsumeiEntry:
    ...
    stripped = text.strip()
    # "NNN name amount" — coded entry with amount
...
def _parse_setsumei_no_full_code(stripped: str) -> SetsumeiEntry:
    # "NNN name" — coded entry without amount
...
def _parse_setsumei_text(stripped: str) -> SetsumeiEntry:
    # "description amount" — text with trailing amount
...
def _is_setsu(idx: CellIndex, row: int) -> bool:
    """Row has 節 pattern in kubun + has kingaku."""
    return (
...
def _is_sub_item(idx: CellIndex, row: int) -> bool:
    """Row has kubun + kingaku, but kubun is not a 節 pattern."""
    return (
        _has_text(idx, row, COL_KUBUN)
        and _has_text(idx, row, COL_KINGAKU)
...
def _is_continuation(idx: CellIndex, row: int) -> bool:
    """Row has kubun text but no kingaku (name continuation of previous row)."""
...
def classify_row(idx: CellIndex, row: int, headers: frozenset[int]) -> str:
    """
    Classify a single row into one of:
      "header" | "moku" | "setsu" | "sub_item" | "continuation" | "setsumei" | "empty"
    """
    return (
...
# Accumulator: (completed_groups, current_moku_row, current_children)
_MokuAcc = tuple[
...
def _make_setsu_step(idx: CellIndex):
    """Curried step function: CellIndex → (acc, row) → acc."""
...
def group_rows_by_setsu(
    ...
    """Group rows by 節 within a 目's children. Returns ((setsu_row|None, child_rows), ...)."""
...
def _continuation_rows(idx: CellIndex, rows: Sequence[int]) -> tuple[int, ...]:
    """Leading continuation rows from a sequence."""
...
def collect_full_name(idx: CellIndex, base_row: int, child_rows: Sequence[int]) -> str:
    """Collect 節 name including continuation rows (e.g., '負担金、補助' + '及び交付金')."""
...
def build_setsu_record(
    ...
    """Assemble a SetsuRecord from a setsu row and its child rows."""
    # Name, number, amount
    full_name = (
    ...
    # Sub-items: kubun + kingaku, not a 節 pattern
    sub_items = tuple(
    ...
    # 説明: collect from setsu row + children
    setsumei_rows = (
...
def build_moku_record(
    ...
    """Assemble a MokuRecord from a 目 row and its child rows."""
    # Right-table rows: include moku row if it has right-table data
    right_rows = (
        (moku_row, *child_rows)
...
def parse_page_budget(cells: Sequence[Cell]) -> PageBudget:
    ...
    Composition:
      cells → index → classify → group by 目 → group by 節 → assemble records
    """
    idx = build_cell_index(cells)
